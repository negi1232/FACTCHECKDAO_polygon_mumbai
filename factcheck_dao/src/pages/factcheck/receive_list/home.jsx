            ...
            <ul className="pagination">
                {currentPage > 0 && (
                    <li className="page-item">
                        <a className="page-link" onClick={() => addQueryParam(currentPage - 1)} href="#">
                            {currentPage - 1}
                        </a>
                    </li>
                )}
...
function RequestFactcheck_list_receive({contracts, reloadKey}) {
    ...
    const [request_fc_list, setRequestFc_list] = useState(undefined);
    ...
    return (
        <div>
            ...
            {request_fc_list !== undefined &&
                request_fc_list.length > 0 &&
                request_fc_list.map(
                    (request_fc, index) =>
                        request_fc.deadline > Math.floor(new Date().getTime() / 1000) && (
                            <Link className="card" key={index} to={"/factcheck/receive/" + request_fc.id} style={{textDecoration: "none"}}>
                                <div className="row">
                                    <div className="col-3">
